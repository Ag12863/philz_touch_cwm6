...
static void change_menu_color() {
    const char* headers[] = { "修改菜单颜色", NULL };
...
static void gestures_action_setup() {
    ...
    for (;;) {
        ...
        while (gesture_action[i] != NULL) {
            ...
            ui_format_gui_menu(item_press_lift, "上滑", gesture_action[press_lift_action.value]);
            ui_format_gui_menu(item_press_move, "下滑", gesture_action[press_move_action.value]);
            i++;
        }
...
void show_touch_gui_menu() {
    ...
    for (;;) {
        if (touch_to_validate.value == NO_TOUCH_SUPPORT)
            ...
            ui_format_gui_menu(item_touch, "Touch GUI", "双击 tap");
        else
        ...
        if (enable_vibrator.value)
            ui_format_gui_menu(item_vibra, "震动", "打开");
        else ui_format_gui_menu(item_vibra, "震动", "关闭");
